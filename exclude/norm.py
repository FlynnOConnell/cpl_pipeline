...
def get_baseline(resp_signal, resp_times, wave_signal, first):
    ...
    respiratory_signal = np.array(
        resp_signal[: int(first_event_time)], dtype=float
    )  # 1 Hz
    respiratory_times = np.array(
        resp_times[: int(first_event_time)], dtype=float
    )  # 1 Hz
    ...
    slowest_breathing_index_spikes_arr = int(
        slowest_breathing_time * 2000
    )  # Conversion factor due to different sampling rates

    # Step 2: Extract corresponding segment from spikes_arr
    segment_data = wave_signal[
        :,
        slowest_breathing_index_spikes_arr : slowest_breathing_index_spikes_arr
        + baseline_size * 2000,
    ]
...
def get_master_df():
    ...
    for animal_path in animals:
        ...
        norm_signal = sorted(
            animal_path.glob("*_respiratory_signal*.npy"), key=extract_common_key
        )
        norm_times = sorted(
            animal_path.glob("*_respiratory_times*.npy"), key=extract_common_key
        )
        ...
        baseline_segment = [
            get_baseline(resp_signal, resp_times, raw_arr, first_event)
            for resp_signal, resp_times, raw_arr, first_event in zip(
                norm_signal, norm_times, raw_data, first_event_times
            )
        ]
    ...
    for animal in all_animals:
        ...
        for context in ["context", "nocontext"]:
            ...
            df = update_df_with_epochs(
                animal_df[animal_df["Context"] == context],
                pre_fmin,
                pre_fmax,
                epoch_tmin,
                epoch_tmax,
            ...
            epochs = list(
                map(lambda e: set_event_id(e), df["epoch"].tolist())
            )  # list, item for each session
            baseline = df["baseline"].tolist()  # list, item for each session

            concatenated_epochs = mne.concatenate_epochs(epochs)
            concatenated_bl = np.concatenate(baseline, axis=1)
            concatenated_bl = concatenated_bl.reshape(
                1, concatenated_bl.shape[0], concatenated_bl.shape[1]
            )
            cells = concatenated_epochs.ch_names

            # elim any dropped channels
            channel_indices = [
                cells.index(ch_name) for ch_name in concatenated_epochs.ch_names
            ]
            ...
            for ch_pair in combinations(range(len(brain_regions_all)), 2):
                ...
                con = compute_coherence_base(
                    concatenated_epochs, ch_pair, np.arange(pre_fmin, pre_fmax)
                )
                coh = np.squeeze(con.get_data())
                connectivity_freqs = np.array(con.freqs)
                time = np.array(con.times)

                con_bl = compute_coherence_base(
                    final_bl, ch_pair, np.arange(pre_fmin, pre_fmax)
                )
                ...
                for domain in ["time_domain", "freq_domain"]:
                    ...
                    for band, (fmin, fmax) in lfp_bands.items():
                        ...
                        idx_bl = np.where(
                            (connectivity_freqs_bl >= fmin)
                            & (connectivity_freqs_bl <= fmax)
                        )[0]
                        ...
                        if domain == "time_domain":
                            ...
                            sem_coh = np.std(coh[idx, :], axis=0) / np.sqrt(
                                coh[idx, :].shape[0]
                            )
                            sem_coh_bl = np.std(coh_bl[idx_bl, :], axis=0) / np.sqrt(
                                coh_bl[idx_bl, :].shape[0]
                            )
                            ...
                            sem_coh = np.std(coh[idx, :], axis=1) / np.sqrt(
                                coh[idx, :].shape[1]
                            )
                            sem_coh_bl = np.std(coh_bl[idx_bl, :], axis=1) / np.sqrt(
                                coh_bl[idx_bl, :].shape[1]
                            )
        ...
        for context in ["context", "nocontext"]:
            ...
            for group in ["between", "within"]:
                ...
                fig, (ax_time, ax_freq, ax_all) = plt.subplots(
                    1, 3, figsize=(15, 6), sharey=True
                )
                ...
                for domain, ax in [("time_domain", ax_time)]:
                    for band_name, coh_data in all_animals_data[animal][context][group][
                        domain
                    ].items():
                        ...
                        if band_name != "all":
                            ...
                            ax.title(
                                f"{band_name}: {domain}, {context}, {group}",
                                fontsize=14,
                                fontweight="bold",
                            )
                ...
                y_mean_all = all_animals_data[animal][context][group]["time_domain"][
                    "all"
                ]["mean"]
                x_all = con.times
                ax_all.imshow(
