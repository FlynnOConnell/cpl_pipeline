...
def make_single_trial_psth(spike_train, win_size, win_step, time=None):
    ...
    psth_time = np.arange(
        np.min(time) + (win_size / 2), np.max(time) - (win_size / 2), win_step
    )
    ...
    with tables.open_file(h5_file, "r") as hf5:
        ...
        for trial in spike_array:
            for i, unit in enumerate(trial):
                tmp, tmp_time = make_single_trial_psth(unit, win_size, win_step, time)
                PSTHs[:, i] += tmp
...
def make_psths_for_tastant(h5_file, win_size, win_step, dig_in_ch, smoothing_width=3):
    ...
    with tables.open_file(h5_file, "r+") as hf5:
        ...
        for ti, trial in enumerate(spike_array):
            for ui, unit in enumerate(trial):
                ...
                if psth_time is None:
                    psth_time = tmp_time
                    PSTHs = np.zeros(
                        (spike_array.shape[1], spike_array.shape[0], len(psth_time))
                    )
        ...
        hf5.create_array("/PSTHs/%s" % dig_str, "mean_psths", np.mean(PSTHs, axis=1))
        hf5.flush()
...
def get_mean_difference(A, B, axis=0):
    ...
    # equation instead
    SEM = np.sqrt((np.power(sd1, 2) / n1) + (np.power(sd2, 2) / n2)) / np.sqrt(n1 + n2)
    # SEM = np.sqrt((np.power(sd1, 2)) + (np.power(sd2,2)))

    return C, SEM


def zscore_to_baseline(time, rates):
    """
    ...
    rates : numpy.array, Trial x Time array of firing rates
    ...
    baselines = np.mean(rates[:, idx], axis=1)
    m = np.mean(baselines)
    sd = np.std(baselines)
...
def remove_baseline(time, rates):
    """
    Takes a firing rate and substracts the group baseline mean from each
    trials' firing rates
    ...
    """
    idx = np.where(time < 0)[0]
    baseline = np.mean(rates[:, idx])
    norm_fr = rates - baseline
    return norm_fr
...
def check_taste_response(time, spikes, win_size=1500):
    ...
    stats = {
        "u-stat": stat,
        "p-val": pval,
        "baseline": (np.mean(pre), sem(pre)),
        "response": (np.mean(post), sem(post)),
        "delta": mean_delta,
    }
