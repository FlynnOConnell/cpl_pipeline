...
### custom QDialog which allows user to fill in ops and run spk2extract!
class RunWindow(QDialog):
    ...
    def create_buttons(self):
        ...
        tooltips = [
            ...
            "apply ROI classifier before signal extract with probability threshold (set to 0 to turn off)",
            ...
            "allow shared pixels to be used for fluorescence extract from overlapping ROIs (otherwise excluded from both ROIs)",
        ...
        saveDef.clicked.connect(self.save_defaults)
        revertDef = QPushButton("Revert default ops to built-in")
        revertDef.clicked.connect(self.revert_defaults)
    ...
    def run_s2e(self):
        ...
        # print(self.db)
        # print("Running spk2extract with command:")
        # cmd = f"-u -W ignore -m spk2extract --ops {ops_file} --db {db_file}"
        # print("python " + cmd)
        # self.process.start(sys.executable, cmd.split(" "))

        #self.process.start('python -u -W ignore -m spk2extract --ops "%s" --db "%s"' %
        #                   (ops_file, db_file))
    ...
    def started(self):
        ...
        save_folder = os.path.join(self.db["save_path0"], "spk2extract/")
    ...
    def finished(self):
        ...
        if self.finish and not self.error:
            ...
            if len(self.opslist) == 1:
                self.parent.fname = os.path.join(self.db["save_path0"], "spk2extract",
                                                 "plane0", "stat.npy")
    ...
    def stdout_write(self):
        ...
        #self.logfile = open(os.path.join(self.save_path, "spk2extract/run.log"), "a")
...
class OpsButton(QPushButton):
    ...
    def __init__(self, bid, Text, parent=None):
        ...
        self.setText(Text)
        self.clicked.connect(partial(self.press, parent, bid))
        self.show()

    def press(self, parent, bid):
        try:
            opsdef = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                                  f"../ops/ops_{parent.opsname[bid]}.npy")
