...
def memory_usage_psutil():
    ...
    MB_conv = 2**20
    return (
        current_mem.rss / MB_conv,
        current_mem.vms / MB_conv,
        current_mem.shared / MB_conv,
    )
...
def report_usage(label="", logfile=sys.stderr):
    ...
    print(
        "Location %s: file = %s , function = %s , line = %s"
        % (label, myfile, myfunction, myline),
        file=logfile,
    )
    print(
        "\trss_mem = %lf MB ,vms_mem = %lf MB ,shared_mem = %lf MB, max_mem = %lf MB"
        % (current_mem + (maxmem,)),
        file=logfile,
    )
    return
