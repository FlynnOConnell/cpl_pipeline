...
from cpl_extract.spk_io.printer import println
...
def create_hdf_arrays(file_name, electrode_mapping, lfp_mapping):
    ...
    with tables.open_file(str(file_name), "r+") as hf5:
        ...
        if not lfp_mapping.empty:
            for idx, row in lfp_mapping:
                hf5.create_earray("/raw_lfp", "lfp%i" % row["name"], atom, (0,))
