...
from cpl_extract.analysis import clustering, spike_analysis as sas
...
def detect_spikes(filt_el, spike_snapshot=[0.5, 1.0], fs=30000.0, thresh=None):
    ...
    waves_dj, times_dj = clustering.dejitter(
        np.array(waves), np.array(times), spike_snapshot, fs
    )
...
class SpikeDetection(object):
    ...
    def __init__(self, file_dir, chan_num, params=None, overwrite=False):
        # Setup paths to files and directories needed
        self._file_dir = file_dir
        self._channel_number = chan_num
        self._out_dir = os.path.join(file_dir, "spike_detection", f"channel_{chan_num}")
        self._data_dir = os.path.join(self._out_dir, "data")
        self._plot_dir = os.path.join(self._out_dir, "plots")
        self._files = {
            "params": os.path.join(
                file_dir, "analysis_params", "spike_detection_params.json"
            ),
    ...
    def run(self):
        ...
        electrode = self._channel_number
        params = self.params
        ...
        if ref_el is None:
            print(
                "Could not find referenced data for electrode %i. Using raw."
                % electrode
            )
            ...
            if ref_el is None:
                raise KeyError(
                    "Neither referenced nor raw data found for electrode %i in %s"
                    % (electrode, file_dir)
                )

        # Filter electrode trace
        filt_el = clustering.get_filtered_electrode(
            ref_el, freq=params["bandpass"], sampling_rate=fs
        )
        del ref_el
        ...
        if not status["recording_cutoff"]:
            ...
            dplt.plot_recording_cutoff(filt_el, fs, self.recording_cutoff, out_file=fn)
        ...
        if not status["pca_waveforms"]:
            ...
            dplt.plot_explained_pca_variance(explained_variance_ratio, out_file=fn)
    ...
    def __str__(self):
        ...
        out.append("Electrode: %i" % self._channel_number)
        ...
        if out_dir is None:
            if len(rec_dirs) > 1:
                ...
                out_dir = os.path.join(
                    rec_dirs[0], "BlechClust", "electrode_%i" % electrode
                )
...
class SpikeSorter(object):
    def __init__(self, rec_dirs, electrode, clustering_dir=None, shell=False):
        ...
        if clustering_dir is None:
            if len(rec_dirs) > 1:
                ...
                clustering_dir = os.path.join(
                    top, "BlechClust", "electrode_%i" % electrode
                )
            else:
                clustering_dir = os.path.join(
                    rec_dirs[0], "BlechClust", "electrode_%i" % electrode
        ...
        try:
            clust = BlechClust(
                rec_dirs, electrode, out_dir=clustering_dir, no_write=True
            )
        except FileNotFoundError:
            clust = None
    ...
    def split_cluster(
        ...
        if target_clust >= len(self._active):
            raise ValueError(
                "Invalid target. Only %i active clusters" % len(self._active)
            )
    ...
    def plot_cluster_waveforms_over_time(self, target_cluster, interval):
        ...
        if len(start_times) > 10:
            userIO.tell_user(
                "This would open more than 10 figures, choose a larger interval"
            )
            return
    ...
    def plot_clusters_wavelets(self, target_clusters):
        ...
        waves = [self._active[i]["spike_waveforms"] for i in target_clusters]
        fig, ax = dplt.plot_waveforms_wavelet_tranform(
    ...
    def plot_clusters_acorr(self, target_clusters):
        ...
        for i in target_clusters:
            cluster = self._active[i]
            acf, bin_centers, edges = sas.spike_time_acorr(
    ...
    def plot_clusters_xcorr(self, target_clusters):
        ...
        for x, y in pairs:
            ...
            xcf, bin_centers, edges = sas.spike_time_xcorr(
