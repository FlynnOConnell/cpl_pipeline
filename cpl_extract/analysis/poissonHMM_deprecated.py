...
def forward(spikes, dt, PI, A, B):
    ...
    row = np.array(
        [PI[i] * np.prod(poisson(B[:, i], spikes[:, 0], dt)) for i in range(nStates)]
    )
    ...
    for t in range(1, nTimeSteps):
        tmp = np.array(
            [
                np.prod(poisson(B[:, s], spikes[:, t], dt))
        ...
        if t < nTimeSteps - 1:
            ...
            for si in range(nStates):
                for sj in range(nStates):
                    ...
                    epsilonNumerator[si, sj] = (
                        alpha[si, t] * A[si, sj] * beta[sj, t] * probs
                    )
...
class TestData(object):
    ...
    def _generate_ground_truth(self):
        ...
        for i, r in enumerate(mean_rates):
            stateRates[i, :] = np.array(
                [r, *np.abs(np.random.normal(r, 0.5 * r, nStates - 1))]
            )
    ...
    def _generate_spike_trains(self):
        ...
        for trial, cell, t in it.product(
            range(nTrials), range(nCells), range(nTimeSteps)
        ):
...
class PoissonHMM(object):
    ...
    def randomize(self):
        ...
        print("Randomizing")
        diag = np.abs(np.random.normal(0.99, 0.01, nStates))
        ...
        B = np.vstack(
            [
                np.abs(np.random.normal(x, y, nStates))
    ...
    def fit(
        ...
        while not self.isConverged(convergence_thresh) and (self.iteration < max_iter):
    ...
    def isConverged(self, thresh):
        ...
        dCost = cost - oldCost
        print(
            "dPI = %f,  dA = %f,  dB = %f, dCost = %f, cost = %f"
    ...
    def _compute_data_rate_array(self):
        ...
        rate_array = convert_spikes_to_rates(
            self.data, self.dt, win_size, step_size=win_size
        )
    ...
    def _compute_predicted_rate_array(self):
        ...
        win_size = self._cost_window
        dt = self.dt
        mean_rates = generate_rate_array_from_state_seq(
    ...
    def _update_cost(self):
        ...
        cost, BIC, bestPaths, maxLogProb = compute_hmm_cost(
            spikes, dt, PI, A, B, win_size=win_size, true_rates=true_rates
        )
        self.cost = cost
        self.BIC = BIC
        self.best_sequences = bestPaths
...
def plot_state_raster(data, stateVec, dt, ax=None):
    ...
    for trial, spikes in enumerate(data):
        ...
        for i, row in enumerate(spikes):
            ...
            ax.scatter(
                time[idx],
                row[idx] * trial + gradient[i],
                c=[colors[int(x)] for x in path[idx]],
                marker="|",
            )
...
def plot_state_rates(rates, ax=None):
    ...
    sns.barplot(x="state", y="rate", hue="cell", data=df, palette="muted", ax=ax)
...
def compare_hmm_to_truth(truth_dat, hmm, state_map=None):
    ...
    ax2[0].set_title(
        ...
        "Mean Correct: %0.1f%%, Median: %0.1f%%" % (mean_correct, median_correct)
    )
    ...
    for i, x in enumerate(zip(truePaths, bestPaths)):
        ...
        while t < nPts:
            ...
            tmp = ax2[1].fill_between(
                [t_start, t_end], [i, i], [i + h, i + h], color=trueCol[s]
            )
        ...
        while t < nPts:
            ...
            t_end = time[t + next_t - 1]
            tmp = ax2[1].fill_between(
                [t_start, t_end], [i, i], [i - h, i - h], color=hmmCol[s]
            )
    ...
    ax2[1].legend(handles, labels, shadow=True, bbox_to_anchor=(0.78, 0.5, 0.5, 0.5))
...
def compute_new_matrices(spikes, dt, gammas, epsilons):
    ...
    A = A / np.sum(A, axis=1)
    Bnumer = np.sum(
        np.array([np.matmul(tmp_y, tmp_g.T) for tmp_y, tmp_g in zip(spikes, gammas)]),
        axis=0,
    )
...
class HmmHandler(object):
    ...
    def write_overview_to_hdf5(self):
        ...
        if hasattr(self, "_fitted_models"):
            ...
            models = dict.fromkeys([x["hmm_id"] for x in data_params])
    ...
    def run(self, parallel=True):
        ...
        results = Parallel(n_jobs=n_cpu, verbose=20)(
            delayed(hmm_fit_mp)(rec_dir, p) for p in fit_params
        )
...
def get_hmm_spike_data(
    rec_dir, unit_type, channel, time_start=None, time_end=None, dt=None
):
    ...
    if dt is not None and curr_dt < dt:
        ...
        raise ValueError(
            "Cannot upsample spike array from %f ms "
...
def read_hmm_from_hdf5(h5_file, hmm_id, rec_dir):
    ...
    with tables.open_file(h5_file, "r") as hf5:
        ...
        tmp = hf5.root[h_str]
        mats = {
            "initial_distribution": tmp["initial_distribution"][:],
...
def write_hmm_to_hdf5(h5_file, hmm_id, hmm):
    ...
    with tables.open_file(h5_file, "a") as hf5:
        ...
        hf5.create_array("/" + h_str, "initial_distribution", hmm.initial_distribution)
...
class HMMFit(object):
    ...
    def get_spike_data(self):
        ...
        spike_array, dt, time = get_spike_data(
            self._rec_dir,
            units,
            p["channel"],
            dt=p["dt"],
...
def hmm_fit_mp(rec_dir, params):
    ...
    spikes, dt, time = get_hmm_spike_data(
        rec_dir, unit_type, channel, time_start=time_start, time_end=time_end, dt=dt
    )
    hmm = PoissonHMM(params["n_states"], spikes, dt)
