...
import matplotlib
from cpl_extract.sort.utils.shader import waveforms_datashader
...
#     ax.fill_between(mean_x, mean_wave - std_wave, mean_wave + std_wave, alpha=0.4)
#     ax.set_xlabel("Samples (%s)" % fs_str)
#     ax.set_ylabel("Voltage (microvolts)")
#     ax.set_title(unit_title)
...
#
#
# def plot_traces_and_outliers(raw_h5, window=60, save_file=None):
#     """
...
#     ----------
#     raw_h5 : str
#         Path to h5_file with raw data
#     """
#     if not os.path.isfile(raw_h5):
#         raise FileNotFoundError("%s not found." % raw_h5)
#
#     with tables.open_file(raw_h5, "r") as hf5:
#         if "/raw" not in hf5:
#             raise ValueError("No raw data in %s" % raw_h5)
#
...
#         idx = np.where(
#             (time >= time[max_idx] - window / 2) & (time <= time[max_idx] + window / 2)
#         )[0]
#
...
#
#         t_idx = np.where((psth_time >= plot_window[0]) & (psth_time <= plot_window[1]))[
#             0
#         ]
...
#     plt.axvline(
#         np.percentile(intra_J3, percent_criterion),
#         linewidth=2,
#         color="black",
#         linestyle="dashed",
#     )
...
#     fig.savefig(os.path.join(save_dir, "J3_distribution.png"), bbox_inches="tight")
#     plt.close("all")
...
#             ax.plot(time, mean_wave - std_wave, linewidth=2.0, color="black", alpha=0.5)
#             ax.plot(time, mean_wave + std_wave, linewidth=2.0, color="black", alpha=0.5)
#             ax.set_xlabel("Time (ms)", fontsize=35)
#
#             ax.set_title(
#                 "%s %s\ntotal waveforms = %i" % (rl, u, waves.shape[0]), fontsize=20
#             )
...
#         fig.savefig(
#             os.path.join(save_dir, "Unit%s_waveforms.png" % unit_name),
#             bbox_inches="tight",
#         )
#         plt.close("all")
#
def plot_cluster_pca(clusters):
    ...
    for i, c in enumerate(clusters):
        ...
        axs[0, 0].scatter(
            pcs[:, 0], pcs[:, 1], alpha=0.4, s=5, color=colors[i], label=str(i)
        )
        axs[0, 1].scatter(
            pcs[:, 0], pcs[:, 2], alpha=0.4, s=5, color=colors[i], label=str(i)
        )
        axs[1, 0].scatter(
            pcs[:, 1], pcs[:, 2], alpha=0.4, s=5, color=colors[i], label=str(i)
        )
...
def plot_waveforms(waveforms, title=None, save_file=None, threshold=None):
    ...
    """
    fig, ax = waveforms_datashader(waveforms, threshold)
...
def plot_spike_raster(spike_times, waveforms, cluster_ids=None, save_file=None):
    ...
    for i, c in enumerate(zip(cluster_ids, spike_times, waveforms)):
        ...
        ax.scatter(c[1], pcs[:, 0], s=5, color=colors[i], label=str(c[0]))
...
def plot_waveforms_pca(waveforms, cluster_ids=None, save_file=None):
    ...
    for i, c in enumerate(zip(cluster_ids, waveforms)):
        ...
        axs[0, 0].scatter(
            pcs[:, 0], pcs[:, 1], alpha=0.4, s=5, color=colors[i], label=str(c[0])
        )
        axs[0, 1].scatter(
            pcs[:, 0], pcs[:, 2], alpha=0.4, s=5, color=colors[i], label=str(c[0])
        )
        axs[1, 0].scatter(
            pcs[:, 1], pcs[:, 2], alpha=0.4, s=5, color=colors[i], label=str(c[0])
        )
...
def plot_waveforms_umap(
    ...
    ax.legend()
    ax.set_title(
        "Waveforms UMAP\nmin_dist=%f, n_neighbors=%i" % (min_dist, n_neighbors)
    )
...
def plot_waveforms_wavelet_tranform(
    ...
    fig, ax = plt.subplots(
        nrows=n_rows, ncols=n_cols, figsize=(5 * (n_cols + 1), 5 * n_rows)
    )
    ax = ax.reshape(ax.size)
    for i, p in enumerate(pairs):
        for x, y, z in zip(data, cluster_ids, colors):
            ax[i].scatter(
                x[:, p[0]], x[:, p[1]], s=3, alpha=0.5, color=z, label=y, marker="o"
            )
...
def plot_cluster_features(data, clusters, x_label="X", y_label="Y", save_file=None):
    """
    Plot scatter of featureX vs featureY for each cluster
    ...
    for i, clust in enumerate(unique_clusters):
        ...
        tmp = ax.scatter(data[idx, 0], data[idx, 1], color=colors[i], s=0.8)
