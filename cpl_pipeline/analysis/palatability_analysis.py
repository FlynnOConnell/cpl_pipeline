from cpl_pipeline.base.objects import load_dataset
from cpl_pipeline.spk_io import h5io
from cpl_pipeline.utils import print_tools as pt, userIO
import numpy as np
import pandas as pd
import tables
...
def palatability_identity_calculations(
    rec_dir, pal_ranks=None, params=None, shell=False
):
    ...
    trial_list = trial_list[
        [True if x in taste_names else False for x in trial_list.name]
    ]
    ...
    if unit_type is None:
        q = userIO.ask_user(
            "Which units do you want to use for taste "
    ...
    if unit_type == "Single":
        chosen_units = unit_table.loc[unit_table["single_unit"], "unit_num"].to_list()
    elif unit_type == "Multi":
        chosen_units = unit_table.loc[
            unit_table["single_unit"] == False, "unit_num"
        ].to_list()
    elif unit_type == "All":
        chosen_units = unit_table["unit_num"].to_list()
    else:
        selection = userIO.select_from_list(
            "Select units to use:",
            unit_table["unit_num"],
            "Select Units",
            multi_select=True,
    ...
    if params is None or params.keys() != default_pal_id_params.keys():
        ...
        params = userIO.confirm_parameter_dict(
            params,
            ("Palatability/Identity " "Calculation Parameters" "\nTimes in ms"),
            shell=shell,
        )
    ...
    print(
        "Running palatability/identity calculations with parameters:\n%s"
    ...
    with tables.open_file(dat.h5_file, "r+") as hf5:
        ...
        bin_times = np.arange(time[0], time[-1] - win_size + win_step, win_step)
        num_bins = len(bin_times)

        palatability = np.empty(
            (num_bins, num_units, num_tastes * num_trials), dtype=int
        )
        identity = np.empty((num_bins, num_units, num_tastes * num_trials), dtype=int)
        unscaled_response = np.empty(
            (num_bins, num_units, num_tastes * num_trials), dtype=np.dtype("float64")
        )
        response = np.empty(
            (num_bins, num_units, num_tastes * num_trials), dtype=np.dtype("float64")
        )
        laser = np.empty((num_bins, num_units, num_tastes * num_trials, 2), dtype=float)
        ...
        for i, row in dim.iterrows():
            ...
            for j, u in enumerate(chosen_units):
                for k, t in enumerate(bin_times):
                    ...
                    unscaled_response[k, j, idx] = np.mean(
                        trains_dig_in[i].spike_array[:, u, t_idx], axis=1
                    )
                    try:
                        lasers[k, j, idx] = np.vstack(
                            (
                                trains_dig_in[i].laser_durations[:],
        ...
        hf5.create_array(
            "/ancillary_analysis", "window_params", np.array([win_size, win_step])
        )
        hf5.create_array("/ancillary_analysis", "bin_times", bin_times)
        hf5.create_array(
            "/ancillary_analysis", "unscaled_neural_response", unscaled_response
        )

        # for backwards compatibility
        hf5.create_array(
            "/ancillary_analysis", "params", np.array([win_size, win_step])
        )
        ...
        for row in unique_lasers:
            tmp_trials = [
                j
                for j in range(num_trials * num_tastes)
        ...
        trials_per_condition = int(
            trials_per_condition[0] / num_tastes
        )  # assumes same number of trials per taste per condition
        print(
            "Detected:\n    %i tastes\n    %i laser conditions\n"
        ...
        neural_response_laser = np.empty(
            (num_conditions, num_bins, num_tastes, num_units, trials_per_condition),
            dtype=np.dtype("float64"),
        )
        taste_cosine_similarity = np.empty(
            (num_conditions, num_bins, num_tastes, num_tastes),
            dtype=np.dtype("float64"),
        )
        taste_euclidean_distance = np.empty(
            (num_conditions, num_bins, num_tastes, num_tastes),
            dtype=np.dtype("float64"),
        )
        ...
        for i, trial in enumerate(trials):
            for j, _ in enumerate(bin_times):
                for k, _ in dim.iterrows():
                    idx = np.where(
                        (trial >= num_trials * k) & (trial < num_trials * (k + 1))
                    )[0]
                    neural_response_laser[i, j, k, :, :] = response[j, :, trial[idx]].T
            ...
            for j, _ in enumerate(bin_times):
                for k, _ in dim.iterrows():
                    for l, _ in dim.iterrows():
                        taste_cosine_similarity[i, j, k, l] = np.mean(
                            cosine_similarity(
                                neural_response_laser[i, j, k, :, :].T,
                                neural_response_laser[i, j, l, :, :].T,
                            )
                        )
                        taste_euclidean_distance[i, j, k, l] = np.mean(
                            cdist(
                                neural_response_laser[i, j, k, :, :].T,
                                neural_response_laser[i, j, l, :, :].T,
                                metric="euclidean",
        ...
        hf5.create_array(
            "/ancillary_analysis", "taste_cosine_similarity", taste_cosine_similarity
        )
        hf5.create_array(
            "/ancillary_analysis", "taste_euclidean_distance", taste_euclidean_distance
        )
        hf5.flush()

        # Taste Responsiveness calculations
        bin_params = [params["num_comparison_bins"], params["comparison_bin_size"]]
        ...
        for i, t in enumerate(new_bin_times):
            places = np.where((bin_times >= t) & (bin_times <= t + bin_params[1]))[0]
            for j, u in enumerate(chosen_units):
                # Check taste responsiveness
                f, p = f_oneway(
                    np.mean(response[places, j, :], axis=0),
                ...
                taste_idx = [
                    np.arange(num_trials * k, num_trials * (k + 1))
                    for k in range(num_tastes)
                ]
                taste_responses = [
                    np.mean(response[places, j, :][:, k], axis=0) for k in taste_idx
                ]
        ...
        hf5.create_array(
            "/ancillary_analysis", "taste_disciminating_neurons", discriminating_neurons
        )
        hf5.create_array(
            "/ancillary_analysis", "taste_responsive_neurons", responsive_neurons
        )
        hf5.create_array(
            "/ancillary_analysis", "taste_responsiveness", taste_responsiveness
        )
        hf5.flush()
        ...
        p_discrim = np.empty(
            (num_conditions, num_bins, num_tastes, num_tastes, num_units),
            dtype=np.dtype("float64"),
        )
        ...
        hf5.create_array("/ancillary_analysis", "p_discriminability", p_discrim)
        hf5.flush()
        ...
        if num_tastes > 2:
            ...
            palatability_rank_order_deduction(
                rec_dir,
                neural_response_laser,
                unique_lasers,
                bin_times,
                [t_start, t_end],
            )
        ...
        for i, t in enumerate(trials):
            for j in range(num_bins):
                for k in range(num_units):
                    ...
                    r_spearman[i, j, k], p_spearman[i, j, k] = spearmanr(
                        ranks, palatability[j, k, t]
                    )
                    r_pearson[i, j, k], p_pearson[i, j, k] = pearsonr(
                        response[j, k, t], palatability[j, k, t]
                    )
                    ...
                    r_isotonic[i, j, k] = model.score(
                        palatability[j, k, t], response[j, k, t]
                    )
                    ...
                    for _, row in dim.iterrows():
                        taste = row.channel
                        samples.append(
                            [trial for trial in t if identity[j, k, trial] == taste]
                        )
                ...
                for ti1, r1 in dim.iterrows():
                    for ti2, r2 in dim.iterrows():
                        ...
                        t2 = r2.channel
                        tmp_trials = np.where(
                            (identity[j, 0, :] == t1) | (identity[j, 0, :] == t2)
                        )[0]
                        idx = [trial for trial in t if trial in tmp_trials]
                        X = response[j, :, idx]
                        Y = identity[j, 0, idx]
                        test_results = []
                        c_validator = StratifiedShuffleSplit(
                            n_splits=10, test_size=0.25, random_state=0
                        )
                        for train, test in c_validator.split(X, Y):
                            model = GaussianNB()
                            model.fit(X[train, :], Y[train])
        ...
        hf5.create_array(
            "/ancillary_analysis", "pairwise_NB_identity", pairwise_identity
        )
        hf5.flush()
    ...
    with open(save_file, "w") as f:
        ...
        for i, ul in enumerate(lasers):
            ...
            for pattern in base_p_patterns:
                ...
                print(
                    "Base pattern: %s, Max pattern: %s, Max avg corr: %g"
                    % (pattern, max_order, np.max(corrs)),
                    file=f,
                )
