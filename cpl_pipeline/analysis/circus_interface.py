...
import subprocess
import pylab as plt
from cpl_pipeline.plot import data_plot as dplt
from cpl_pipeline.analysis.cluster import get_ISI_and_violations, get_waveforms
...
class circus_clust(object):
    def __init__(self, file_dir, data_name, sampling_rate, emap):
        ...
        self._cluster_file = os.path.join(
            self._results_dir, data_name + ".clusters.hdf5"
        )
    ...
    def prep_for_circus(self):
        ...
        set_config_parameter(
            self._param_file, "data", "mapping", os.path.basename(self._probe_file)
        )
        set_config_parameter(self._param_file, "data", "sampling_rate", self._fs)
        emap = self._emap
        dead_channels = (
            emap[emap["dead"]].groupby("CAR_group")["Electrode"].unique().to_dict()
        )
        dead_channels = {int(i + 1): list(j) for i, j in dead_channels.items()}
        set_config_parameter(
            self._param_file, "detection", "dead_channels", repr(dead_channels)
        )
        ...
        if os.path.isfile(self._circus_map_file):
            ...
            self._circus_channel_map = pd.read_json(
                self._circus_map_file, orient="records"
            )
        else:
            new_map = (
                emap[emap["dead"] == False]
                .reset_index(drop=True)
                .reset_index()
                .rename(columns={"index": "circus_channel"})
            )
    ...
    def _make_probe_file(self, radius=100):
        ...
        channel_group = {
            int(i + 1): {
                "channels": list(bundles[i]),
                "graph": [],
                "geometry": {
                    j: [i * 2 * radius, k * 2 * radius]
    ...
    def _get_clusters(self, channel_num=None, electrode_num=None):
        ...
        if electrode_num is None:
            electrode_num = ch_map[ch_map["circus_channel"] == channel_num][
                "Electrode"
            ].values[0]

        if channel_num is None:
            channel_num = ch_map[ch_map["Electrode"] == electrode_num][
                "circus_channel"
            ].values[0]
