...
from cpl_pipeline import spk_io
...
from cpl_pipeline.spk_io import printer as pt, writer, userio
import os
    ...
    for rd in rec_dirs:
        ...
        for un in unit_names:
            ...
            if descrip["single_unit"] == 1:
                ...
                tmp_J3 = calc_J3(pca_waves[:idx1, :], pca_waves[idx2:, :])
...
def find_held_units(rec_dirs, percent_criterion=95, rec_names=None, raw_waves=False):
    ...
    held_df = pd.DataFrame(
        columns=["unit", "electrode", "single_unit", "unit_type", *rec_names, "J3"]
    )
        ...
        for unit1 in unit_names1:
            ...
            if descrip1["single_unit"] == 1:
                for unit2 in unit_names2:
                    if raw_waves:
                        wf2, descrip2, fs2 = h5io.get_raw_unit_waveforms(
...
def resolve_duplicate_matches(found_cells):
    ...
    for row in go_back:
        idx = np.where(
            (found_cells[:, 0] == row[0][0]) & (found_cells[:, 1] != row[0][1])
        )[0]
        if len(idx) == 1:
            new_found.append(found_cells[idx, :])
            continue
...
def get_response_change(
    ...
    time1, spikes1 = spk_io.h5io.get_spike_data(rec1, unit1, din1)
    time2, spikes2 = spk_io.h5io.get_spike_data(rec2, unit2, din2)
