...
from cpl_pipeline.utils.particles import HMMInfoParticle
from cpl_pipeline import load_dataset
from cpl_pipeline.spk_io import h5io
from cpl_pipeline.utils import math_tools as mt
...
)  # tried increasing to 50, something bad happened
...
def poisson(rate, n, dt):
    ...
    tmp[
        rate > 150
    ] = MIN_PROB  # cap rate at 150, since neurons don't really fire that fast
...
def forward(spikes, dt, PI, A, B):
    ...
    a0 = [
        np.exp(np.log(PI[i]) + log_emission(B[:, i], spikes[:, 0], dt))
        for i in range(nStates)
    ]
    a0 = np.array(a0)
...
def compute_new_matrices(spikes, dt, gammas, epsilons):
    ...
    for si in range(n_states):
        for sj in range(n_states):
            ...
            if np.isfinite(Adenom) and Adenom != 0.0:
...
def poisson_viterbi_deprecated(spikes, dt, PI, A, B):
    ...
    T1[:, 0] = np.array(
        [
            np.log(PI[i]) + np.log(np.prod(poisson(B[:, i], spikes[:, 1], dt)))
            for i in range(nStates)
        ]
    )
...
def poisson_viterbi(spikes, dt, PI, A, B):
    ...
    T1[:, 0] = [
        np.log(PI[i]) + sum_log_probs(poisson(B[:, i], spikes[:, 0], dt))
...
def compute_hmm_cost(spikes, dt, PI, A, B, win_size=0.25, true_rates=None):
    if true_rates is None:
        true_rates = convert_spikes_to_rates(spikes, dt, win_size, step_size=win_size)

    BIC, bestPaths, maxLogProb = compute_BIC(PI, A, B, spikes=spikes, dt=dt)
    hmm_rates = generate_rate_array_from_state_seq(
        bestPaths, B, dt, win_size, step_size=win_size
    )
...
def compute_best_paths(spikes, dt, PI, A, B):
    ...
    for i, trial in enumerate(spikes):
        bestPaths[i, :], pathProbs[i], _, _ = poisson_viterbi(trial, dt, PI, A, B)
...
def rebin_spike_array(spikes, dt, time, new_dt):
    ...
    dbins = int(
        new_dt / dt
    )  # keep in mind that "time" is actually an index of integers
...
def get_hmm_spike_data(
    ...
    if dt is not None and curr_dt < dt:
        ...
        raise ValueError(
            "Cannot upsample spike array from %f sec "
...
def query_units(dat, unit_type, area=None):
    ...
    Parameters
    ----------
    dat : cpl_pipeline.dataset or str
...
def fit_hmm_mp(rec_dir, params, h5_file=None, constraint_func=None):
    ...
    for ch, tst in zip(channels, tastes):
        if custom_trial_nums is None:
            tmp_s, _, time = get_hmm_spike_data(
                rec_dir,
                unit_type,
                ch,
    ...
    if h5_file:
        ...
        try:
            ...
            if old_hmm is None:
                ...
                print(
                    "%s: Existing HMM %s found. Comparing log likelihood ..."
                    % (pid, hmm_id)
                )
...
def roll_back_hmm_to_best(hmm, spikes, dt, thresh):
    ...
    diff_ll = np.diff(filt_ll)
    below = (
        np.where(np.abs(diff_ll) < thresh)[0] + 1
    )  # since diff_ll is 1 smaller than ll_hist
    ...
    tmp = [
        x
        for x in below
        if check_ll_trend(hmm, thresh, n_iter=iterations[x]) == "plateau"
    ]
    if len(tmp) != 0:
        below = tmp
    ...
    maxima = iterations[
        below[maxima]
    ]  # this is the iteration at which the maxima occurred
...
class PoissonHMM(object):
    ...
    def randomize(self, spikes, dt, time, row_id=None, constraint_func=None):
        ...
        diag = np.abs(np.random.normal(0.99, 0.01, n_states))
        ...
        B = np.vstack(
            [
                np.abs(np.random.normal(x, y, n_states))
        ...
        self._init_history()
        self.stat_arrays["gamma_probabilities"] = self.get_gamma_probabilities(
            spikes, dt
        )
    ...
    def fit(self, spikes, dt, time, max_iter=500, threshold=1e-5, parallel=False):
        ...
        if (
            self.initial_distribution is None
            or self.transition is None
            or self.emission is None
        ):
            raise ValueError(
                "Must first initialize fit matrices either manually or via randomize"
            )
        ...
        while not converged and (self.iteration < max_iter):
            ...
            if (
                last_logl is not None
                and np.isfinite(delta_ll)
                and delta_ll < threshold
                and np.isfinite(self.fit_LL)
                ...
                converged = True
                print(
                    "%s: %s: Change in log likelihood converged"
                    % (os.getpid(), self.hmm_id)
                )
            ...
            print(
                "%s: %s: Iter #%i complete. Log-likelihood is %.2E. Delta is %.2E"
    ...
    def _step(self, spikes, dt, parallel=False):
        ...
        results = Parallel(n_jobs=n_cores, backend="multiprocessing")(
            delayed(baum_welch)(trial, dt, PI, A, B) for trial in spikes
        )
    ...
    def get_forward_probabilities(self, spikes, dt, parallel=False):
        ...
        a_results = Parallel(n_jobs=n_cpu, backend="multiprocessing")(
            delayed(forward)(trial, dt, PI, A, B) for trial in spikes
        )
    ...
    def get_backward_probabilities(self, spikes, dt, parallel=False):
        ...
        b_results = Parallel(n_jobs=n_cpu, backend="multiprocessing")(
            delayed(backward)(trial, dt, A, B, n) for trial, n in zip(spikes, norms)
        )
    ...
    def get_gamma_probabilities(self, spikes, dt, parallel=False):
        ...
        results = Parallel(n_jobs=n_cpu, backend="multiprocessing")(
            delayed(baum_welch)(trial, dt, PI, A, B) for trial in spikes
        )
...
class HmmHandler(object):
    def __init__(self, dat, save_dir=None):
        """Takes a cpl_pipeline dataset object and fits HMMs for each tastant

        Parameters
        ----------
        dat: cpl_pipeline.dataset
        ...
        if save_dir is None:
            save_dir = os.path.join(dat.root_dir, "%s_analysis" % dat.data_name)
    ...
    def get_overview_w_AIC(self):
        ...
        ov["AIC"] = (ov["BIC"] + 2 * ov["max_log_prob"]) / np.log(
            ov["n_time_steps"]
        ) - 2 * ov["max_log_prob"]
        return ov
    ...
    def remove_params(self, index):
        ...
        if params["fitted"]:
            print(
                f"Parameter set {index} is already fitted. "
    ...
    def run(
        self, parallel=True, overwrite="unconverged", constraint_func=None, n_cpu=None
    ):
        h5_file = self.h5_file
        ...
        assert (
            1 <= n_cpu <= max_cpu
        ), f"n_cpu must be a valid integer 1 <= n_cpu <= {max_cpu}"

        results = Parallel(n_jobs=n_cpu, verbose=100, backend="multiprocessing")(
            delayed(fit_hmm_mp)(rec_dir, p, h5_file, constraint_func)
            for p in fit_params
        )
    ...
    def plot_spike_rasters(self):
        ...
        for i, row in data.iterrows():
            ...
            spikes, dt, time = get_hmm_spike_data(
                rec_dir,
                params["unit_type"],
                params["channel"],
                time_start=params["time_start"],
    ...
    def plot_euclidean_distance_matrix(self):
        ...
        distance_df = pd.DataFrame(
            distance_matrix, index=emissions.columns, columns=emissions.columns
        )
        sns.heatmap(
            distance_df, cmap="viridis", cbar_kws={"label": "Euclidean distance"}
        )
    ...
    def add_params(self, params):
        ...
        cpl_pipeline.analysis.poisonHMM.HMM_PARAMS, any undefined parameters will be
        ...
        for k, v in HMM_PARAMS.items():
            if k not in params.keys():
                params[k] = v
                print(
                    "Parameter %s not provided. Using default value: %s" % (k, repr(v))
                )
        ...
        if single_taste:
            for t in tastes:
                ...
                if t not in dim.index:
                    print(
                        "Taste %s not found in dig_in_mapping or marked to exclude. Skipping..."
                        % t
                    )
                    continue
            ...
            p = params.copy()
            if p["n_trials"] is None:
                p["n_trials"] = sum([len(trials.query("name == @t")) for t in tastes])
...
class ConstrainedHMM(PoissonHMM):
    ...
    def randomize(self, spikes, dt, time, row_id=None, constraint_func=None):
        ...
        baseline = np.mean(np.sum(spikes[:, :, b_idx], axis=2), axis=0) / (
            len(b_idx) * dt
        )
        ...
        PI = np.hstack(
            [
                [np.random.normal(unit, 0.02, 1)[0] for x in range(n_baseline)],
                np.zeros((n_tastes * 2,)),
            ]
        )
        PI = PI / np.sum(PI)
    ...
    with tables.open_file(save_file, "a") as hf5:
        for i, row in df.iterrows():
            ...
            if "spikes" not in hf5.root[path_str]:
                spikes, dt, spike_time = get_hmm_spike_data(
                    rd,
                    params["unit_type"],
                    channel,
                    time_start=params["time_start"],
            ...
            hf5.create_array(
                d_str, "state_probabilities", stat_arrays["gamma_probabilities"]
            )
        ...
        for k, v in kwargs.items():
            if isinstance(v, pd.DataFrame):
                ...
                print(
                    f"Invalid datatype for {k}. Must be pandas.DataFrame. Not written to HDF5"
                )
