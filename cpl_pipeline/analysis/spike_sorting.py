...
def make_spike_arrays(h5_file, params):
    ...
    if dig_in_ch is None or dig_in_ch == []:
        raise ValueError(
            "Must provide dig_ins_to_use in params in " "order to make spike arrays"
        )
    # get digital input table
    dig_in_table = h5io.read_trial_data_table(h5_file, "in", dig_in_ch)

    # Get laser input table
    if laser_ch is not None or laser_ch == []:
        laser_table = h5io.read_trial_data_table(h5_file, "in", laser_ch)
        lasers = True
        ...
        for i in dig_in_ch:
            ...
            for ti, trial_off in enumerate(on_idx):
                ...
                for unit in hf5.root.sorted_units:
                    ...
                    spike_idx = np.where(
                        (unit.times[:] >= window[0]) & (unit.times[:] <= window[1])
                    )[0]
                ...
                if lasers:
                    ...
                    for li, l in enumerate(laser_ch):
                        tmp_trial = laser_table.query(
                            "abs(off_index - " "@trial_off) <= " "@post_idx"
                        )
                        if not tmp_trial.empty:
                            ...
                            duration = (
                                tmp_trial["off_index"] - tmp_trial["on_index"]
                            ) / (fs / 1000)
                            ...
                            lag = (tmp_trial["on_index"] - trial_off) / (fs / 1000)
            ...
            time = hf5.create_array(
                "/spike_trains/dig_in_%i" % i, "array_time", array_time
            )
            tmp = hf5.create_array(
                "/spike_trains/dig_in_%i" % i, "spike_array", np.array(spike_train)
            )
            hf5.flush()

            if lasers:
                ld = hf5.create_array(
                    "/spike_trains/dig_in_%i" % i, "laser_durations", cond_array
                )
                ls = hf5.create_array(
                    "/spike_trains/dig_in_%i" % i, "laser_onset_lag", laser_start
                )
                ol = hf5.create_array(
                    "/spike_trains/dig_in_%i" % i, "on_laser", laser_single
                )
                hf5.flush()
...
def calc_units_similarity(h5_file, fs, similarity_cutoff=50, violation_file=None):
    ...
    if violation_file is None:
        violation_file = os.path.join(
            os.path.dirname(h5_file), "unit_similarity_violations.txt"
        )
    ...
    with tables.open_file(h5_file, "r+") as hf5:
        ...
        for pair in itertools.product(units, repeat=2):
            ...
            print("Computing similarity between Unit %i and Unit %i" % (u1_idx, u2_idx))
    ...
    if violations > 0:
        ...
        for x, y in violation_pairs:
            ...
            out_str += "   {:<10}{:<10}{}\n".format(x, y, unit_distances[u1][u2])
