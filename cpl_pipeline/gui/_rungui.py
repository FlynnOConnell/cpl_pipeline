...
### custom QDialog which allows user to fill in ops and run cpl_pipeline!
class RunWindow(QDialog):
    ...
    def run_s2e(self):
        ...
        # print(self.db)
        # print("Running cpl_pipeline with command:")
        # cmd = f"-u -W ignore -m cpl_pipeline --ops {ops_file} --db {db_file}"
        # print("python " + cmd)
        # self.process.start(sys.executable, cmd.split(" "))

        # self.process.start('python -u -W ignore -m cpl_pipeline --ops "%s" --db "%s"' %
        #                   (ops_file, db_file))
    ...
    def started(self):
        ...
        save_folder = os.path.join(self.db["save_path0"], "cpl_pipeline/")
        ...
        if self.finish and not self.error:
            ...
            if len(self.opslist) == 1:
                self.parent.fname = os.path.join(
                    self.db["save_path0"], "cpl_pipeline", "plane0", "stat.npy"
                )
    ...
    def stdout_write(self):
        ...
        # self.logfile = open(os.path.join(self.save_path, "cpl_pipeline/run.log"), "a")
