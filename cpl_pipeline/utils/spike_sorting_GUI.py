...
from cpl_pipeline.utils import tk_widgets as tkw
...
class SpikeSorterGUI(ttk.Frame):
    ...
    def initUI(self):
        ...
        solution_drop = ttk.OptionMenu(solution_row, self._solution_var, *solutions)
        ...
        splitUMAP = ttk.Button(
            buttons, text="UMAP Split (Slow)", command=self.umap_split_clusters
        )
    ...
    def split_clusters(self, *args, umap=False):
        ...
        new_clusts = self.sorter.split_cluster(
            chosen,
            params["n_iterations"],
            params["n_restarts"],
            params["thresh"],
        ...
        popup = tkw.ListSelectPopup(
            choices,
            self.root,
            "Select split clusters to keep.\n" "Cancel to undo split.",
            multi_select=True,
        )
        self.disable_all()
        self.root.wait_window(popup.root)
...
def make_waveform_plot(wave, wave_std, n_waves=None, index=None):
    ...
    ax.text(xlim[1] - 200, ylim[0] + 0.1 * abs(ylim[0]), tmp_str, fontsize=10)

    if index is not None:
        ax.text(
            xlim[0] + 10,
            ylim[0] + 0.05 * abs(ylim[0]),
            str(index),
            fontweight="bold",
            fontsize=14,
            )
