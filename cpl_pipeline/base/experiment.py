...
from cpl_pipeline import load_params
from cpl_pipeline.analysis.cluster import CplClust, UMAP_METRICS, SpikeSorter
from cpl_pipeline.analysis.held_unit_analysis import find_held_units
from cpl_pipeline.base.objects import data_object, load_dataset
from cpl_pipeline.plot.data_plot import plot_held_units, plot_J3s
from cpl_pipeline.spk_io import h5io, print_dict, print_dataframe
from cpl_pipeline.spk_io.paramio import write_params_to_json
from cpl_pipeline.spk_io import userio
from cpl_pipeline.utils.spike_sorting_GUI import launch_sorter_GUI
...
class experiment(data_object):
    ...
    def __init__(self, exp_dir=None, exp_name=None, shell=False, order_dict=None):
        ...
        super().__init__(
            "experiment", root_dir=exp_dir, data_name=exp_name, shell=shell
        )
    ...
    def _change_root(self, new_root=None):
        ...
        self.recording_dirs = [
            x.replace(old_root, new_root) for x in self.recording_dirs
        ]
        self.rec_labels = {
            k: v.replace(old_root, new_root) for k, v in self.rec_labels.items()
        }

        self.analysis_dir = self.analysis_dir.replace(old_root, new_root)
        return new_root
    ...
    def __str__(self):
        ...
        out.append(print_dict(self.rec_labels, tabs=1))
        out.append("\nTaste Mapping :")
        out.append(print_dict(self.taste_map, tabs=1))
        out.append("\nElectrode Mapping\n----------------")
        out.append(print_dataframe(self.electrode_mapping))
        if hasattr(self, "held_units"):
            out.append("\nHeld Units :")
            out.append(print_dataframe(self.held_units.drop(columns=["J3"])))
    ...
    def _order_dirs(self, shell=False, order_dict=None):
        ...
        if order_dict is None:
            self.recording_dirs = [
                x[:-1] if x.endswith(os.sep) else x for x in self.recording_dirs
            ]
            top_dirs = {
                os.path.basename(x): os.path.dirname(x) for x in self.recording_dirs
            }
            ...
            file_dirs = [k for k, v in order_dict.items() if v is not None and v != 0]
            file_dirs = sorted(file_dirs, key=order_dict.get)
            file_dirs = [os.path.join(top_dirs.get(x), x) for x in file_dirs]
        else:
            file_dirs = sorted(
                self.recording_dirs, key=order_dict.get
            )  # TODO: idk what is hapening with these key orders this is fucked
    ...
    def detect_held_units(self, percent_criterion=95, raw_waves=False, shell=False):
        ...
        held_df, intra_J3, inter_J3 = find_held_units(
            rec_dirs, percent_criterion, raw_waves=raw_waves
        )
        ...
        self.J3_values = {"intra_J3": intra_J3, "inter_J3": inter_J3}
        ...
        rec_pairs = [
            (rec_names[i], rec_names[i + 1]) for i in range(len(rec_names) - 1)
        ]
        with open(df_file, "w") as f:
            for rec1, rec2 in rec_pairs:
                ...
                print(print_dataframe(tmp_df, tabs=1), file=f)
                print("", file=f)
        ...
        plot_held_units(rec_dirs, held_df, save_dir, rec_names=rec_names)
    ...
    def cluster_spikes(
        ...
        print("\nRunning Blech Clust\n-------------------")
        print("Parameters\n%s" % print_dict(clustering_params))
        ...
        for rd in rec_dirs:
            ...
            write_params_to_json("clustering_params", rd, clustering_params)
            # dat.save()

        # Run clustering
        if not umap:
            clust_objs = [
                CplClust(rec_dirs, x, params=clustering_params)
                for x in electrodes
            ]
        else:
            clust_objs = [
    ...
    def sort_spikes(self, electrode=None, shell=False):
        ...
        if not shell:
            root, sorter_GUI = launch_sorter_GUI(sorter)
    ...
    def get_electrode_unit_counts(self):
        ...
        out = (
            all_unit_table.groupby(["electrode", "rec"])
            .size()
