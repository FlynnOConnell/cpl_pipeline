...
from cpl_pipeline.analysis.poissonHMM import HmmHandler
from cpl_pipeline.base.objects import data_object, load_experiment, load_dataset
from cpl_pipeline.spk_io.writer import read_dict_from_json, write_dict_to_json
...
class project(data_object):
    def __init__(
        ...
        self._files = {
            "params": os.path.join(
                self.root_dir, self.data_name + "_analysis_params.json"
            )
        }
        ...
        if status["params"] and params is None:
            self._params = read_dict_from_json(self._files["params"])

        elif params is not None:
            self._params = params
            write_dict_to_json(params, self._files["params"])
        else:
    ...
    def remove_experiment(self, exp_name):
        ...
        if len(idx) == 0:
            ...
            print(
                "Dropping experiment %s from project.\n%s\nRemoved"
                % (exp_name, df.loc[idx])
            )
    ...
    def export_portable_copy(self, dest_dir=None, extensions=None):
        ...
        confirmation = (
            input("Do you want to copy the files to the above directory? (y/n): ")
            .strip()
            .lower()
        )
        if confirmation != "y":
