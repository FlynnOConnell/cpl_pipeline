...
from cpl_pipeline.spk_io import printer as pt
...
def get_files(prompt="", root=None, filetypes=None, multi=False, shell=False):
    ...
    if filetypes is not None:
        file_filter = lambda x: (
            any([x.endswith(y) for y in filetypes]) or os.path.isdir(x)
        )
    else:
        file_filter = None
    ...
    while go:
        if shell:
            ...
            tmp = eg.fileopenbox(
                prompt + " (cancel to stop collection)",
                default=root,
                multiple=multi,
                filetypes=filetypes,
            )
...
class dictIO:
    def __init__(self, data, types=None, shell=False):
        ...
        if not isinstance(data, Mapping):
            raise TypeError(
                (
                    "%s is invalid data type. Requires extension of"
                    "Mapping such as dict."
                )
                % type(data)
            )
        if types is None:
            ...
            raise ValueError(
                (
                    "types must be a dict with keys matching data"
    ...
    def fill_dict_gui(self, prompt=None):
        ...
        if prompt:
            ...
            ttk.Separator(root, orient="horizontal").pack(
                side="top", fill="x", expand=True, pady=10
            )
